...
function read_date($date) {
  return date("d/m/Y", strtotime($date)); // Convierte a DD/MM/YYYY
}
...
/*--------------------------------------------------------------*/
/* Function for Finding product by barcode
/*--------------------------------------------------------------*/
function find_by_barcode($barcode) {
    global $db;
    $sql = "SELECT * FROM products WHERE barcode = '{$db->escape($barcode)}' LIMIT 1";
