...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Error!</b> The group name actually exists in the database.');
   ...
   if(empty($errors)){
        ...
        if($db->query($query)){
          ...
          //failed
          $session->msg('d','Unfortunately the group could not be created!');
          redirect('add_group.php', false);
        }
   } else {
